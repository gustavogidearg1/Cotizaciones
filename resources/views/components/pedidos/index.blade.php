                            ...
                            <form action="{{ route('pedidos.index') }}" method="GET">
                                <div class="input-group">
                                    <input type="text" name="search" class="form-control"
                                        placeholder="Buscar pedidos..." value="{{ request('search') }}">
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            @foreach ($pedidos as $pedido)
                                <tr>
                                    <td>{{ $pedido->id }}</td>
                                    <td class="text-nowrap">{{ \Carbon\Carbon::parse($pedido->fecha)->format('d/m/Y') }}
                                    </td>
                                    ...
                                    <td>
                                        @if ($pedido->subPedidos->isNotEmpty())
                                            ...
                                            @if ($pedido->subPedidos->count() > 1)
                                                <span
                                                    class="badge bg-secondary ms-1">+{{ $pedido->subPedidos->count() - 1 }}
                                                    más</span>
                                            @endif
                                        @else
                                            N/A
